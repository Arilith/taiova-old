...
export class BusLogic {
  ...
  public async GetBus(company : string, vehicleNumber: string, small = false, subCompany = false) {
    try { 
      if(subCompany) return await this._busDal.GetBusBySubCompany(company, vehicleNumber);
